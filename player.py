...
class Player(Tk.Frame):
    ...
    def __init__(self, parent, title=None):
        ...
        self.ua_text = Tk.Text(ctrlpanel3, height=5)
        self.ua_text.pack()

        #  english text
        self.en_text = Tk.Text(ctrlpanel3, height=5)
        self.en_text.pack()

        ctrlpanel3.pack(side=Tk.BOTTOM, fill=Tk.X, expand=1)
    ...
    def OnOpen(self):
        ...
        self.OnStop()
        self.en_text.insert(Tk.INSERT, '')
        self.ua_text.insert(Tk.INSERT, '')
        ...
        if os.path.isfile(fullname):
            ...
            ua_subtitle_file_name = str(os.path.join(dirname, file_name_without_extension + '_ua.txt'))
            en_subtitle_file_name = str(os.path.join(dirname, file_name_without_extension + '_en.txt'))

            ua_text = open(ua_subtitle_file_name, 'r')
            self.ua_text.insert(Tk.INSERT, ua_text.read())
            ua_text.close()

            en_text = open(en_subtitle_file_name, 'r')
            self.en_text.insert(Tk.INSERT, en_text.read())
            en_text.close()
